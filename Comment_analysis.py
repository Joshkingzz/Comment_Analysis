...
#function to check if a comment contains a website link
...
#Extracting app names from comment and creating a separate table where we have app names and comment made on the apps
import pandas as pd
import re

#getting my source file
url = r"C:\UDEMY\DATA ANALYSIS\PYTHON\1-python basics\My Project\Web_comment.csv"
df = pd.read_csv(url)
df.drop(['0'],axis=1,inplace=True) #dropping unwanted columns
def extract_urls(comment): #Function to extract URLs
    url_pattern = r'(http[s]?://\S+|www\.\S+)' #Re for URLs
    urls = re.findall(url_pattern, comment)
    return ', '.join(urls) if urls else None


df['extracted_urls'] = df['Comments'].apply(extract_urls) #Create a new column with extracted URLs
